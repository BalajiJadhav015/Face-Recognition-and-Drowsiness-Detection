...
class face_identifier:
    ...
    def __init__(self):
        ...
        with open(r'C:\Users\lenovo\Desktop\Face_Recognition_Drowsiness_Detection\encodings.pickle', 'rb') as file:
            self.Knownencodesdict = pickle.load(file)


    def MarkAttendance(self, name, status):
        df = pd.read_csv(r'C:\Users\lenovo\Desktop\Face_Recognition_Drowsiness_Detection\attendance.csv')
        now = datetime.now()
        date = now.strftime("%d-%m-%Y")
        time = now.strftime("%H:%M:%S")
        new_row = {'Name': name, 'Date' : date, 'Time' : time, 'Drowsiness_Status': status} 
        new_df = df.append(new_row, ignore_index=True)
        new_df.to_csv(r'C:\Users\lenovo\Desktop\Face_Recognition_Drowsiness_Detection\attendance.csv', index = False)
    ...
    def drowsiness_detection(self, our_img):
        ...
        face_landmark_path = r'shape_predictor_68_face_landmarks.dat'
